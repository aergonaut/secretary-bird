...
async function check_title(context: Context, config: Config) {
  ...
  if (!title_passes_check) {
    let summary = []
    summary.push("The Pull Request title does not match the repository's configured PR title format.");
    summary.push(`The required format is: \`${pull_request_title_regex}\``);
